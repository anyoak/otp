...
def parse_phone_number(phone_str):
    ...
    try:
        ...
        # This is a fallback for numbers without country codes
...
# ---------------------------
# HELPERS - ENHANCED CASE VARIATIONS WITH PATTERNS
# ---------------------------
def generate_variations(email):
    """Generate email variations with enhanced case patterns and dots"""
    try:
        if "@" not in email:
            return []
        ...
        # Pattern 7: CamelCase style (first letter of each "word" if we had words)
        # Since we don't have words, we'll uppercase every 3rd character as a pattern
        ...
        # Generate dot variations for each case pattern
        for case_variant in case_patterns:
            # Add without dots
            variations.add(case_variant + "@" + domain)
            
            # Generate variations with dots between characters
            if len(case_variant) > 1:
                # Add single dot at each possible position
                for i in range(1, len(case_variant)):
                    dotted = case_variant[:i] + "." + case_variant[i:]
                    variations.add(dotted + "@" + domain)
                
                # Add multiple dots (1-3 dots total)
                for num_dots in range(1, min(4, len(case_variant))):
                    for positions in itertools.combinations(range(1, len(case_variant)), num_dots):
                        dotted = []
                        for i, char in enumerate(case_variant):
                            dotted.append(char)
                            if i in positions:
                                dotted.append(".")
                        variation = "".join(dotted) + "@" + domain
    ...
    try:
        ...
        if variations:
            response_text = (
                ...
                f"💡 <b>Algorithm:</b> Enhanced case patterns with dots\n\n"
...
async def start_handler(message: types.Message):
    ...
    welcome_text = (
        ...
        "🔹 <b>Dot variations</b> - Dots between characters\n"
...
async def help_handler(message: types.Message):
    ...
    help_text = (
        ...
        "• Enhanced case patterns and dot variations\n"
